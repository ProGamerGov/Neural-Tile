...
main(){
	...
	# 3. Chop the styled image into 1x2 tiles with the specified overlap value.
	out_dir=$output/$clean_name
	mkdir -p $out_dir
	convert $out_file -crop 1x2+"$overlap_w"+"$overlap_h"@ +repage +adjoin $out_dir/$clean_name"_%d.png"
	...
	convert -background transparent \( $feathered_dir/$clean_name'_0.png' -background transparent \) \
			\( $feathered_dir/$clean_name'_1.png' -background transparent \) \
	...
	convert \( $tiles_dir/$clean_name'_0.png' \) \
			\( $tiles_dir/$clean_name'_1.png' \) \
